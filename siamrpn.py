...
class SiameseAlexNet(nn.Module):
    ...
    def forward(self,template,detection):
        ...
        detection__feature=self.featureExtract(detection)
        ...
        kernel_regression=kernel_regression.view(N,4*self.anchor_num,256,4,4)
        conv_score=self.conv_cls2(detection__feature)
        conv_regression=self.conv_r2(detection__feature)
        ...
        pred_score=F.conv2d(conv_scores,score_filters,groups=N)
        pred_score=pred_score.reshape(N,10,self.score_displacement+1,self.score_displacement+1)
        ...
        pred_reg=F.conv2d(conv_reg,reg_filters,groups=N)
        pred_reg=pred_reg.reshape(N,20,self.score_displacement+1,self.score_displacement+1)
        pred_regression=self.regress_adjust(pred_reg)
        #pred_score: (8,10,17,17) tensor float32 pred_regression:(8,20,17,17),tensor float32
